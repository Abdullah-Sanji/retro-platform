import { defineComponent, ref, mergeProps, useSSRContext, computed, onMounted, unref } from "vue";
import { ssrRenderAttrs, ssrRenderStyle, ssrRenderAttr, ssrRenderList, ssrRenderClass, ssrIncludeBooleanAttr, ssrLooseEqual, ssrInterpolate, ssrLooseContain, ssrRenderComponent } from "vue/server-renderer";
import { useRouter, useRoute } from "vue-router";
import { u as useMutation, a as api, b as useNotification, c as useQuery, J as JoinModal } from "./marketing-sqXyumEb.js";
import { R as RetroBoard } from "./retro-Gi2zJJjc.js";
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const templateType = ref("went_well_to_improve_actions");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div><label class="block text-sm font-semibold text-gray-700 mb-2"> Votes Per User </label><input${ssrRenderAttr("value", votesPerUser.value)} type="number" min="1" max="10" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"><p class="text-sm text-gray-500 mt-1">Each participant will have ${ssrInterpolate(votesPerUser.value)} votes</p></div><div class="border-2 border-gray-200 rounded-lg p-4"><div class="flex items-center mb-3"><input${ssrIncludeBooleanAttr(Array.isArray(timerEnabled.value) ? ssrLooseContain(timerEnabled.value, null) : timerEnabled.value) ? " checked" : ""} type="checkbox" id="timer-checkbox" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"><label for="timer-checkbox" class="ml-2 text-sm font-semibold text-gray-700"> ⏱️ Enable Timer for Collecting Phase </label></div>`);
