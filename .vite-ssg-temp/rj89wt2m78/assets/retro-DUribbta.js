...
import { u as useMutation, a as api, b as useNotification, c as useQuery, _ as _export_sfc, d as useFullPermission } from "./marketing-1kkeCTA4.js";
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const isSessionActive = computed(() => {
      var _a, _b;
      return ((_b = (_a = props.sessionData) == null ? void 0 : _a.session) == null ? void 0 : _b.isActive) !== false && props.currentPhase !== "completed";
    });
    return (_ctx, _push, _parent, _attrs) => {
      var _a;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex items-center gap-3" }, _attrs))}><div class="flex gap-2"><!--[-->`);
      ssrRenderList(phases, (phase) => {
        _push(`<button${ssrIncludeBooleanAttr(!isSessionActive.value) ? " disabled" : ""} class="${ssrRenderClass([
          "px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-md",
          !isSessionActive.value ? "opacity-50 cursor-not-allowed" : "transform hover:scale-105",
      ...
      _push(`<!--]--></div><div class="h-10 w-px bg-gradient-to-b from-transparent via-gray-300 to-transparent"></div>`);
      if (isSessionActive.value) {
        _push(`<button class="px-5 py-2 bg-gradient-to-r from-red-500 to-rose-500 text-white rounded-xl text-sm font-semibold hover:from-red-600 hover:to-rose-600 transition-all shadow-lg hover:shadow-xl transform hover:scale-105"> End Session </button>`);
      } else {
        _push(`<div class="px-5 py-2 bg-gray-400 text-white rounded-xl text-sm font-semibold shadow-lg"> Session Ended </div>`);
      }
      _push(`<div class="px-4 py-2 bg-gradient-to-r from-amber-400 to-yellow-400 text-amber-900 rounded-xl text-xs font-bold shadow-lg flex items-center gap-2"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg> Facilitator </div>`);
